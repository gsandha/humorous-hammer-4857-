...
export default function Main() {
  ...
  return (
    ...
    <SimpleGrid columns={{ sm: 1, md: 2, lg: 3 }} textAlign="center" gap={4}>
      {DataMain.map((el) => (
        <GridItem backgroundColor="#fff" paddingBottom={4}>
          ...
          <Flex gap={3} paddingRight={5} justifyContent="end" marginTop="-36px">
            ...
            >
              {" "}
              <BsHeart color="red" fontSize="28px" />
            </Stack>
            <Stack
              boxShadow="rgba(0, 0, 0, 0.16) 0px 1px 4px"
              borderRadius="50%"
            ...
            >
              {" "}
              <BsWhatsapp color="limegreen" fontSize="28px" />{" "}
            </Stack>
          </Flex>
          ...
          >
            <Text>
              By <span style={{ color: "black" }}>{el.title}</span>{" "}
            </Text>
